...
```
Ứng dụng được thiết kế để chạy hoàn toàn riêng tư:
- Tất cả downstream AWS service calls đều qua VPC Endpoint (Bedrock, S3, Secrets Manager)
- Không có NAT Gateway trên network
- Mọi instance/Lambda đều trong private subnet
- Không có yêu cầu outbound internet trực tiếp

Vì thế egress firewall không cần thiết. Security Group + NACL đủ để ép buộc.

Tuy nhiên, nếu trong production scale lên cần fetch data từ external API hoặc update model lên 
Hugging Face, sẽ trigger deploy Network Firewall trước NAT Gateway.
```
...
IngressRules:
  - IpProtocol: tcp
    FromPort: 22
    ToPort: 22
    CidrIp: 0.0.0.0/0          # ❌ SSH mở toàn bộ internet
```
...
IngressRules:
  - IpProtocol: tcp
    ...
    Description: "SSH từ Bastion host chỉ"

EgressRules:
  - IpProtocol: tcp
    FromPort: 443
    ToPort: 443
    DestinationPrefixListId: pl-bedrock-vpce  # VPC Endpoint cho Bedrock
```

**Screenshot - Updated SG (no 0.0.0.0/0 on 22):**
![Updated SG](./images/w5-mh2-sg-hardened.png)
...
```yaml
NetworkAcl: acl-app-private
Rules:
  - RuleNumber: 50
    Protocol: -1              # Tất cả protocols
    RuleAction: DENY
    CidrBlock: 192.168.0.0/16
    Description: "DENY từ corporate network (bạn không dùng)"
  
  - RuleNumber: 100
    Protocol: -1
    RuleAction: ALLOW
    CidrBlock: 10.0.0.0/8
    Description: "ALLOW từ VPC internal"
```

**Screenshot - NACL Rules:**
![NACL Rules](./images/w5-mh2-nacl-rules.png)
